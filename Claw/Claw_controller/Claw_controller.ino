/*class ClawClass
  ...
  };*/                                    // ignore this I'm just playing with objects
int _buttonState = 1 ;//
int _servoPosition;
int _grabCount;
int _teller;
...
/*  Een grijper die open of dicht gaat naar aanleiding van de waarde die middels een draaiknop wordt gegeven (en middels MAP functie omgevormd tot een bruikbare waarde)
    Als een object gegrepen is:   dan luidt er een buzzer.    En  Het ledje gaat knipperen        Hoe kleiner de hoek is van de grijper, hoe feller het ledje gaat branden
    Op de LCD wordt weergeven:
    de hoek van de motor(dus de grijper) = Vraagt de waarde op van de huidige grijperwaarde       Hoe vaak een object is gepakt = Telt        En geeft weer indien een object wordt vastgehouden.*/
void setup() {}

void loop()
{
  analogRead (_buttonState);
  _servoPosition = map(_buttonState, 0, 1023, 0, 180);                            /*" funtie: map(value, fromLow, fromHigh, toLow, toHigh)*/
